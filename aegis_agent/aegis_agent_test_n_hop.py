...
class AegisAgent:
    ...
    def __init__(
        ...
        self.use_text_mode = False  # 控制是否使用文本模式而非图片模式
        # 初始化工具
        self._init_tools()
        ...
        import aegis_agent.agent_tools
        aegis_agent.agent_tools.AGENT_INSTANCE = self
    ...
    def _llm_node(self, state: State):
        ...
        # # 使用完整函数调用图，测试n hop用的
        # with open(f"prompts/test_n_hop.user_prompt.{self.language}.md", "r") as f:
        #     user_prompt = f.read()

        if self.use_text_mode:
            # 文本模式：将调用图信息转换为文本描述
            graph_text = self._graph_info_to_text()
            # print(graph_text)
            user_input = HumanMessage(content=[
                {"type": "text", "text": f"{graph_text}"}
            ])
        else:
            # 图片模式：使用PNG图片
            print(f"Load PNG: {self.current_png_path}")
    ...
    def _graph_info_to_text(self) -> str:
        """读取对应目录下的 .dot 文件，提取关键内容并过滤无关标签"""
        if not self.current_png_path:
            return "无调用图信息"

        # 将 .png 后缀替换为 .dot
        dot_path = self.current_png_path.replace(".png", ".dot")
        ...
        try:
            ...
            return f"未找到对应的 .dot 文件: {dot_path}"
        except Exception as e:
            return f"读取 .dot 文件失败: {e}"

        # 替换颜色
        for color, replacement in hex_to_color.items():
            content = content.replace(color, replacement)
        ...
        # 提取图标题
        match = re.search(r'Vulnerability Path:.*?"', content, re.DOTALL)
        title = match.group(0).replace('"', '')
        if match:
            lines.append(f"Figure Title: {title}")
        ...
        # 提取节点信息：节点名和其 label 中的文字内容
        node_pattern = r'"([^"]+)"\s*\[label=<<TABLE[^>]*>(.*?)</TABLE>>'
        for match in re.finditer(node_pattern, content, re.DOTALL):
            ...
            # 提取表格中所有文字内容，去掉标签后拼接
            text_content = strip_tags(table_content)
            if text_content:
                lines.append(text_content)
            lines.append("")

        # 提取边（调用关系）
        edge_pattern = r'"([^"]+)"\s*->\s*"([^"]+)"\s*\[label=([^]\s]+)'
        for match in re.finditer(edge_pattern, content):
            caller = match.group(1)
            callee = match.group(2)
            label = match.group(3).strip('"')
            lines.append(f"Edge: {caller} -({label})-> {callee}")
    ...
    def _double_check(self, messages):
        ...
        def _get_result_confidence(keyword, text_with_probs):
            # [todo] 所以这里其实也是让LLM自己去报confidence，其实有些不准确，LLM会报得虚高
    ...
    def analyze_current_graph(
        ...
        Returns:
            分析结果
        """
        self.use_text_mode = use_text  # 启用文本模式
        ...
        if not self.use_text_mode:
            ...
            with open(f"prompts/analyze_paths_textual.system_prompt.{self.language}.md", "r") as f:
                system_prompt = f.read()
        ...
        self.use_text_mode = False  # 恢复图片模式
        return result["messages"][-1].content
    

    def analyze_n_hop_on_current_graph(
        self, 
        recursion_limit: int = 100,
        use_text: bool = False
    ) -> Dict[str, Any]:
        ...
        ctx = get_context()
        with open(ctx.get("prompt_template_path", "Unknown"), "r") as f:
            system_prompt = f.read()

        formatted_system_prompt = system_prompt.replace(
                "{func1}", str(ctx.get("start_node", "Unknown"))
            ).replace(
                "{func2}", str(ctx.get("end_node", "Unknown"))
            ).replace(
                "{n}", str(ctx.get("n_hops", 0))
            )

        sys_input = SystemMessage(content=formatted_system_prompt)
    ...
    def analyze_current_graph_for_dpi_selection(
        self, 
        recursion_limit: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        prompt_template = ctx.get("prompt_template_path", "Unknown")
        version = int(os.path.basename(prompt_template).split('.')[0].split('_v')[-1])
        with open(prompt_template, "r", encoding='utf-8') as f:
            system_prompt = f.read()

        if 'withNode' in prompt_template:
            extracted_path_file = f"evaluations/extracted_paths/{ctx.get('png_name').split('_dpi_')[0]}.json"
            if not os.path.exists(extracted_path_file):
                print(f"[000] No such file: {extracted_path_file}")
                return

            with open(extracted_path_file, "r", encoding='utf-8') as f:
                node_list = json.load(f)
            if 'all_nodes' not in node_list:
                print(f"[001] 'all_nodes' not in {extracted_path_file}")
                return
            node_list = node_list['all_nodes']
            system_prompt = system_prompt.replace('[all_node_list]', json.dumps(node_list, ensure_ascii=False))
        if version >= 3:
            test_node_list = ctx.get('task_nodes', [])

            if not len(test_node_list):
                logger.warning('task_nodes is NULL, please check random select')
                return None

            for i in range(len(test_node_list)):
                system_prompt = system_prompt.replace(f'[Target_Node_{i+1}]', test_node_list[i])
